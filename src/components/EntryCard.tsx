      ...
      <div className="flex items-center justify-between p-3 bg-base-200 rounded-lg hover:bg-base-300 transition-colors">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className={`badge badge-xs ${statusColors[status]}`}>{status}</span>
          </div>
          <p className="text-sm text-base-content/70 truncate">{entry.phrase}</p>
        </div>
    ...
    <div className="card bg-base-200 shadow-md hover:shadow-lg transition-shadow">
      <div className="card-body p-4">
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {entry.isCustom && <span className="badge badge-info badge-sm">Custom</span>}
            </div>
            <h3 className="card-title text-xl mt-2">{entry.noun}</h3>
          </div>
          <button
            className={`btn btn-sm btn-ghost ${isBookmarked ? 'text-warning' : ''}`}
            onClick={handleBookmark}
          >
            {isBookmarked ? '★' : '☆'}
          </button>
        ...
        <div className="mt-3 space-y-2">
          <p className="font-medium text-primary">{entry.phrase}</p>
        ...
        {(onEdit || onDelete) && (
          <div className="card-actions justify-end mt-3">
            {onEdit && (
              <button className="btn btn-sm btn-outline" onClick={() => onEdit(entry)}>
                Edit
              </button>
            )}
            {onDelete && (
